...
// get user info
twitter.get('/users/show', {screen_name: 'shaun17712185'}, function(err, user, res){
  if(err){console.log('problem getting user request')}

  // request for tweets
  twitter.get('/statuses/user_timeline', {screen_name: 'shaun17712185'}, function(err, tw, res){
    if(err){console.log('problem with tweet request')}

    // request for friends list
    twitter.get('/friends/list', {screen_name: 'shaun17712185'}, function(err, following, res){
      if(err){ console.log('problem with friend list request')}

      // request for direct messages
      twitter.get('/direct_messages/events/list', {screen_name: 'shaun17712185'}, function(err, dm, res){
